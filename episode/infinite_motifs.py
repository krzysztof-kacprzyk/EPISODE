...
def get_default_motif_class(motif, single=False):
    ...
    if single:
        return infinite_motif_single_classes[motif]
    else:
...
class ZZSingle(InfinitiveMotif):
    # described by Ae^(Bt) + C
    # y0 = A + C
    def __init__(self):
        super().__init__()
...
class PPFSingle(InfinitiveMotif):
    # described by Ae^(Bt) + C
    # y0 = A + C
    def __init__(self):
        super().__init__()
    ...
    def predict(self, T):
        return self.A * self.exp(self.B * (T - self.X0)) + self.C
...
class PMPSingle(InfinitiveMotif):
    # described by A*log(1 + x)+B
    # could possibly add multiplication of t as additional parameter
    # could consider powers of x with fraction exponent
    # y0 = B
    def __init__(self):
        super().__init__()
    ...
    def predict(self, T):
        return self.A * self.log(1+T-self.X0) + self.Y0
...
class PMHSingle(InfinitiveMotif):
    # described by A/(1+e^(-B(t+C)))+D
    # y0 = A/(1+e^(-BC)) + D
    def __init__(self):
        super().__init__()
    ...
    def set_by_network(self, properties, x0, y0, y0dt, y0dt2):
        ...
        self.X0 = x0
        self.Y0 = y0
        self.D = y0 - self.A/(1+self.exp(-self.B*self.C))

    def set_by_properties(self, properties, x0, y0, y0dt, y0dt2):
        self.ApD = properties[0]
        self.B = properties[1]
        self.C = properties[2]
        self.A = (self.ApD - y0) / (1 - 1/(1+self.exp(-self.B*self.C)))
        self.D = self.ApD - self.A
        self.X0 = x0

    def predict(self, T):
        return self.A/(1+self.exp(-self.B*(T-self.X0+self.C))) + self.D
...
class MPFSingle(InfinitiveMotif):
    # described by -A*log(1+x)+B
    # y0 = B
    def __init__(self):
        super().__init__()
...
class MPHSingle(InfinitiveMotif):
    # described by Ae^(-Bt) + C
    # y0 = A + C
    def __init__(self):
        super().__init__()
    ...
    def set_by_network(self, properties, x0, y0, y0dt, y0dt2):
        self.B = properties[:,[0]]
        self.C = y0 - properties[:,[1]]
        self.A = y0 - self.C
        self.X0 = x0
        self.Y0 = y0

    def set_by_properties(self, properties, x0, y0, y0dt, y0dt2):
        self.B = self.log(2)/properties[1]
        self.C = properties[0]
        self.A = y0 - self.C
        self.X0 = x0
        self.Y0 = y0
    ...
    def extract_properties_list(self):
        return [self.C,self.log(2)/self.B]
    
    def num_network_properties(self):
        return 2
    
class MMFSingle(InfinitiveMotif):
    # described by -Ae*(Bt) + C
    # y0 = C-A
    def __init__(self):
        super().__init__()

...
class CubicModelNumpy:
    def __init__(self, composition, t_range, x0, t, y):
        ...
        if x0 is None:
            self.x0_included = False
        else:
            self.x0_included = True
    ...
    def get_first_derivative_at_end_from_weights(self):
        sign = 1 if self.composition[-1][0] == '+' else -1
        last_first_derivative = sign * (MIN_ABS_DERIVATIVE + softplus(self.arguments['first_derivative_at_end']))
        return last_first_derivative
     
    def get_second_derivative_at_end_from_weights(self):
        sign = 1 if self.composition[-1][1] == '+' else -1
        last_second_derivative = sign * (MIN_ABS_DERIVATIVE + softplus(self.arguments['second_derivative_at_end']))
    ...
    def extract_coordinates_finite_composition(self):
        ...
        if self.infinite_composition:
            if len(self.composition) > 1:
                ...
                t_last_finite_transition_point = self.t_range[0] + MIN_TRANSITION_POINT_SEP*len(self.composition) + sigmoid(t_last_finite_transition_point) * (1-MIN_RELATIVE_DISTANCE_TO_LAST_FINITE_TRANSITION_POINT) * empirical_range
            else:
        ...
        if len(self.composition) > 1:
            ...
            trans_horizontal_values = softmax(horizontal_values) * scale
            ...
            while np.min(trans_horizontal_values) < MIN_TRANSITION_POINT_SEP:
                ...
                new_horizontal_values = softmax(horizontal_values[good_indices]) * scale
        ...
        vertical_values = softplus(vertical_values) + MIN_VERTICAL_SEPARATION
        ...
        for i in range(len(self.composition)):
            ...
            if utils.is_unbounded_motif(self.composition[i]):
                ...
                if last_first_derivative_status == "zero":
                    ...
                    last_first_derivative = self.get_first_derivative_at_end_from_weights()
                else:
                ...
                if last_second_derivative_status == "zero":
                    ...
                    last_second_derivative = self.get_second_derivative_at_end_from_weights()
                else:
...
def calculate_loss(composition, t_range, x0, t, y, seed=0, train_on_all_data=False, evaluate_on_all_data=True, method='L-BFGS-B'):
    ...
    def loss_function(parameters):
        ...
        ty = np.array([t_train, y_train]).T
        ty_pred = np.array([t_train, y_pred]).T
        distance, path = fastdtw(ty, ty_pred, radius=1)
        return distance
        # return np.mean((y_pred - y_train)**2)

    def loss_function_2(parameters):
        arguments = from_parameters_to_arguments(parameters, dictionary)
        y_pred = model.set_arguments(arguments)
        y_pred = model.forward(t_train)
        return np.mean((y_pred - y_train)**2)
    ...
    try:
        ...
        result = minimize(loss_function_2, result.x, method=method)
    except Exception as e:
        print(e)
        result = minimize(loss_function_2, initial_guess, method=method)
    ...
    final_loss = np.nan_to_num(final_loss, nan=INF)
    
    return final_loss, model


def calculate_loss_with_animation(composition, t_range, x0, t, y,seed=0, train_on_all_data=False, evaluate_on_all_data=True, method='L-BFGS-B', ax=None):
    ...
    all_preds = []
    t_to_plot = np.linspace(t_range[0], t_range[1], 100)
    
    def callback(xk):
        arguments = from_parameters_to_arguments(xk, dictionary)
        y_pred = model.set_arguments(arguments)
        y_pred = model.forward(t_to_plot)
        all_preds.append(y_pred)
     # Use scipy to minimize the loss function

    # Use scipy to minimize the loss function
    result = minimize(loss_function, initial_guess, method=method, callback=callback)
    result = minimize(loss_function_2, result.x, method=method, callback=callback)

    # result = minimize(loss_function_2, result.x, method=method, callback=callback)

    # result = minimize(loss_function_3, initial_guess, method=method, callback=callback)
    ...
    return final_loss, model, all_preds

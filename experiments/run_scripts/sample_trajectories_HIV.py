...
composition_libraries = get_inductive_bias_composition_libraries("HIV")
subtype = "biases_more"
...
chosen_indices = [16,46]

for number, index in enumerate(chosen_indices):

    plt.figure()

    plt.scatter(T[index], Y_true[index][:,0], label='T (observed)')
    plt.plot(new_T[index], Y_pred[index][:,0], label='T (predicted)')
    plt.scatter(T[index], Y_true[index][:,1], label='I (observed)')
    plt.plot(new_T[index], Y_pred[index][:,1], label='I (predicted)')
    plt.scatter(T[index], Y_true[index][:,2], label='V (observed)')
    plt.plot(new_T[index], Y_pred[index][:,2], label='V (predicted)')

    plt.legend()

    plt.savefig(f"results/HIV_plots/HIV_{number}.png", bbox_inches = 'tight')
    plt.savefig(f"results/HIV_plots/HIV_{number}.svg", bbox_inches = 'tight')
    plt.savefig(f"results/HIV_plots/HIV_{number}.pdf", bbox_inches = 'tight')
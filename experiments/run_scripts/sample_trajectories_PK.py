...
composition_libraries = get_inductive_bias_composition_libraries("pk")
subtype = "biases_more"
...
chosen_indices = [17,38]

for number, index in enumerate(chosen_indices):
    plt.figure()

    plt.scatter(T[index], Y_true[index][:,0], label='Concentration (observed)')
    plt.plot(new_T[index], Y_pred[index][:,0], label='Concentration (predicted)')

    plt.legend()

    plt.savefig(f"results/PK_plots/PK_{number}.png", bbox_inches = 'tight')
    plt.savefig(f"results/PK_plots/PK_{number}.svg", bbox_inches = 'tight')
    plt.savefig(f"results/PK_plots/PK_{number}.pdf", bbox_inches = 'tight')